...
class SmallMap {
 public:
    constexpr SmallMap() :
        _arr() {}
    constexpr SmallMap(std::array<std::pair<Key, Value>, N> arr) :
        _arr(arr) {}
    ...
    constexpr auto end() const { return _arr.end(); }
    constexpr auto cend() const { return _arr.cend(); }
    ...
    bool operator==(const SmallMap<Key, Value, N>& other) const { return _arr == other._arr; }
